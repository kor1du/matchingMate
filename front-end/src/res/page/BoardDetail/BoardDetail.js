...
const BoardDetail = () => {
  ...
  const handleInChat = () => {
    if (!board.myPost) {
      ...
      axios
        .post(
          "http://localhost:8080/matchingPost/detail/joinChat",
  ...
  const handleReport = (e) => {
    ...
    axios
      .post("http://localhost:8080/report/create", reportInfo, {
        headers: {
          Authorization: "Bearer " + token,
        },
      })
  ...
  const getBoard = async () => {
    const res = await (
      await axios.get(`http://localhost:8080/matchingPost/detail/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
          ...
          <section className={styles.header}>
            ...
            <div className={styles.userAndDate}>
              ...
              <div className={styles.registerDate}>
                ...
                {!board.myPost && (
                  <div>
                    <Button variant="outlined" onClick={() => setReportModalOpen(true)}>
                      신고
                    </Button>
                  </div>
                )}
