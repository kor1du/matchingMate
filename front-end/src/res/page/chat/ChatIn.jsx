...
function ChatIn() {
  ...
  const chatStart = () => {
    ...
    axiosGet("/chat/in/" + state.roomId, header)
      .then((res) => {
      ...
      waitForConnection(stomp, function () {
        stomp.subscribe(
          ...
          (data) => {
            ...
            if (
              JSON.parse(data.body).sender === "server" &&
              String(JSON.parse(data.body).message).substring(0, 7) === "매칭시간 : "
            ) {
              setIsCompleted(1);
            }
          },
        ...
        // stomp.send("/pub/chat/enter", { Authorization: token }, JSON.stringify({ roomId: roomId, token: token }));
      });
    });
  }
